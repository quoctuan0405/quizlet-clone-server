...
export class UserResolver {
    ...
    async sets(@Parent() user: User) {
        return this.userService.set.findMany({where: {authorId: user.id}, include: {_count: {select: {terms: true}}}});
    ...
    async learningTerms(@Parent() user: User, @Context() context: Ctx) {
        ...
        return this.userService.userLearningTerm.findMany({where: {userId: user.id}});
    }

    @Mutation('signup')
    async signup(@Args('signupInput') signupInput: SignupDTO, @Context() context: Ctx) {
    ...
    @Mutation('login')
    async login(@Args('loginInput') loginInput: LoginInputDTO, @Context() context: Ctx) {
