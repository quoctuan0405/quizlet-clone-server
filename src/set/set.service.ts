...
@Injectable()
export class SetService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
    async onModuleInit() {
        await this.$connect();
    }
    ...
    async findMany() {
        return this.set.findMany({ include: { _count: { select: { terms: true } } } });
    }
                            ...
                            some: {
                                OR: [
                                    ...
                                    { answer: { contains: queryLowercase, mode: 'insensitive' } },
                                    {
                                        options: {
                                            some: {
                                                option: { contains: queryLowercase, mode: 'insensitive' }
                                            }
                                        }
                                    }
                                ]
    ...
    async initUserLearningSet(setId: string | number, context: Ctx) {
        ...
        const alreadyLearningSet = await this.userLearningTerm.findFirst({ where: { userId: context.req.user.id, termId: termsId[0].id } });
