...
export class SetResolver {
    constructor(private setServices: SetService, private termService: TermService) {}
    ...
    async setUserLearningSet(@Args('setId') setId: string, @Context() context: Ctx) {
        // Check if user logged in
        if (!context.req.user) {
            throw new Error("Unauthorized!");
        }
    ...
    @Mutation('createSet')
    async createSet(@Args('createSetInput') createSetInput: CreateSetInput, @Context() context: Ctx) {
        // Check if user logged in
        if (!context.req.user) {
            throw new Error("Unauthorized!");
        }
    ...
    @Mutation('updateSet')
    async updateSet(@Args('updateSetInput') updateSetInput: UpdateSetInput, @Context() context: Ctx) {
        // Check if user logged in
        if (!context.req.user) {
            throw new Error("Unauthorized!");
        }
        ...
        const author = await this.setServices.set.findUnique({where: {id: parseInt(updateSetInput.id)}, select: {authorId: true}});
            ...
            if (term.id) {
                await this.setServices.term.update({
                    ...
                    data: {
                        ...
                        options: {
                            createMany: {
                                data: term.options ? term.options.map(({ option }) => ({ option })) : []
                            }
                        }
                    }
                });
